    ...
    for size in itemsets.keys():
        ...
        for itemset in itemsets[size].keys():
            ...
            for removed in itertools.combinations(itemset, 1):
                ...
                if conf >= min_confidence or set(itemset) == set(query_node):
                    yield Rule(
                        lhs,
                        removed,
                        count(itemset),
            ...
            yield from _ap_genrules(
                itemset, H_1, itemsets, min_confidence, num_transactions,query_node
            )
...
def _ap_genrules(
    ...
    for h_m in H_m:
        ...
        if (count(itemset) / count(lhs)) >= min_conf or set(itemset) == set(query_node):
            yield Rule(
                lhs,
                h_m,
                count(itemset),
    ...
    if H_m_copy:
        yield from _ap_genrules(
            itemset, H_m_copy, itemsets, min_conf, num_transactions,query_node
        )
