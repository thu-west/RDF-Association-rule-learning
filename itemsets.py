...
class _Counter(_ItemsetCounter):
    ...
    def large_itemsets(self, counts, min_support, num_transactions,query_node):
        res=[]
        for (i, c) in counts.items():
            if (c / num_transactions) >= min_support:
                res.append((i,c))
            elif isinstance(i, collections.Iterable) and set(i).issubset(query_node):
                res.append((i,c))
            elif not isinstance(i,collections.Iterable) and i in query_node:
                res.append((i,c))
        return res
        '''
        return [
            (i, c)
            for (i, c) in counts.items()
            if (c / num_transactions) >= min_support or set(i).issubset(query_node)
        ]
        '''
...
class _CounterWithIds(_ItemsetCounter):
    ...
    def large_itemsets(self, counts, min_support, num_transactions, query_node):
        ...
        return [
            (i, count)
            for (i, count) in counts.items()
            if (count.itemset_count / num_transactions) >= min_support or set(i).issubset(query_node)
        ]
        '''
...
def itemsets_from_transactions(
    ...
    large_itemsets = counter.large_itemsets(
        counts, min_support, num_transactions,query_node
    )
    ...
    while large_itemsets[k - 1] and (max_length != 1):
        ...
        C_k = counter.large_itemsets(counts, min_support, num_transactions,query_node)
